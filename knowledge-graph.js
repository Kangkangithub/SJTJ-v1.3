...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // === 分析图表模块开始 ===
// ✅ 替换国家排行图为 "武器类型分布" 可视化内容
// ✅ 修正版本：从 Weapon 节点关联的 IS_TYPE_OF 关系中提取武器类型
function generateAnalysisCharts(data) {
    ...
    // 遍历所有使用关系：Weapon → 使用 → Country
    

    new Chart(document.getElementById('nodeTypeDistributionChart'), {
        type: 'pie',
        data: {
    ...
    new Chart(document.getElementById('weaponYearChart'), {
        type: 'bar',
        data: {
    ...
    // 渲染柱状图
    new Chart(document.getElementById('countryWeaponChart'), {
    type: 'bar',
    data: {
        labels: ['中国', '美国', '俄罗斯', '以色列', '法国', '德国'],
        datasets: [{
            label: '使用武器数量',
            data: [18, 24, 15, 10, 6, 5],
            backgroundColor: '#3498db',
            borderRadius: 4
        }]
    },
