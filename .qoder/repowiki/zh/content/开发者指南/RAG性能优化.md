...
**更新** 本次优化重点围绕"少调LLM、并行调LLM"的核心思路，通过上限控制、并发池、去LLM化与流式增强，显著降低了问答延迟并提升了稳定性。
...
**更新** 新增了n-gram本地关键词提取、受控并发池、向量检索补充等关键优化特性。
...
R->>G : answer(question)
G->>G : extractKeywords() (n-gram本地计算)
...
**更新** 流程图展示了n-gram本地关键词提取、向量检索补充、并发池处理等关键优化步骤。
...
- **关键词提取优化**：从n-gram本地切词替代LLM抽取，避免一次网络往返，且结果确定。使用2-6字滑动窗口，过滤停用词，限制输出数量。
- **图检索**：两轮策略（精确名称→扩展功效/描述/拼音），记录执行过的Cypher便于调试。
- **图遍历扩展**：获取性味归经、方剂关联、配伍冲突，丰富上下文。
- **LLM知识增强优化**：对前N味药做专业信息补全，采用受控并发池（默认3个worker），避免串行拖慢。
...
**更新** 并发池设计确保最多3个worker同时处理药材补全，显著提升吞吐量。
...
- **流式RAG优化**：SSE推送状态、上下文、流式答案片段，并在中间接入向量检索补充，提升召回质量。
...
**更新** 流式端点现在也集成了向量检索补充，解决了之前只走字面匹配的问题。
...
### 瓶颈定位
原流程中"逐味药材LLM补全"是主要耗时点，存在串行与全量两个问题。

### 核心优化策略

#### 1. LLM调用减少策略
- **上限控制**：每次最多对前N味药做LLM补全（ENRICH_HERB_LIMIT = 5），避免无限膨胀。
- **去LLM化**：用本地n-gram提取关键词，减少一次LLM调用，语义匹配由向量检索承担。
- **缓存机制**：24小时增强的药材信息缓存，避免重复调用。

#### 2. 并发处理优化
- **受控并发池**：将串行改为受控并发（ENRICH_CONCURRENCY = 3个worker），显著缩短补全流程时间。
- **防限流保护**：固定并发度避免触发DeepSeek速率限制。

#### 3. 向量检索增强
- **双路检索**：在非流式和流式端点都接入向量检索，弥补CONTAINS字面匹配的语义缺口。
- **智能合并**：语义命中优先，CONTAINS命中去重后追加，提升召回质量。

### 效果对比
以典型问题"肾虚可以吃什么中药调理？"为例：
...
### 后续优化方向
1. **向量检索结果缓存**：对相同query缓存向量检索结果
2. **常用药材补全预热**：离线预计算常用药材的LLM增强信息
3. **流式端点作为默认**：前端默认切换到流式端点
4. **DeepSeek参数调优**：降低max_tokens、优化prompt进一步压缩单次耗时
...
**更新** 新增了n-gram关键词提取和并发池相关的故障排查要点。
...
**更新** 优化后的系统在保证检索准确性的同时，将响应时间从37秒降低到12-15秒，LLM调用次数减少79%，大幅提升了用户体验和系统效率。
