...
## 更新摘要
**所做更改**
- 增强了向量检索机制，改进了RAG管线中的语义检索集成
- 优化了嵌入生成流程，提升了批量处理和错误处理能力
- 完善了相似度搜索实现，增加了更精确的余弦相似度计算
- 更新了数据持久化机制，支持增量同步和单条更新
- 增强了错误处理和降级策略
...
**更新** 增强了向量检索的鲁棒性和性能，改进了错误处理和降级策略，确保在各种情况下都能提供稳定的语义检索服务。
...
- 向量服务（EmbeddingService）
  ...
  - **更新** 增强了错误处理机制，包括API密钥验证、网络异常处理、解析失败降级等。
- RAG 管线（RAGServiceV2）
  - 关键词提取 → Neo4j 精确/模糊检索 → 向量检索补充（步骤 1.5）→ 图遍历扩展上下文 → LLM 知识增强 → 生成答案。
  - **更新** 改进了向量检索的集成逻辑，增加了更完善的回退策略和状态跟踪。
...
**更新** 增强了系统的容错能力，在网络异常或API不可用时自动降级到原有检索模式。
...
- 设计要点
  ...
  - 批量上限 10 条，适配模型限制。
  - **更新** 增强的错误处理：缺失 API Key、网络异常、解析失败均记录日志并降级。
  - **更新** 改进的缓存机制：内存Map存储向量数据，提升检索性能。
...
- **更新** 改进的回退策略：向量检索失败或未就绪时，继续走原有流程，不影响回答。
- **更新** 增强的状态跟踪：详细记录每一步的执行状态和结果。
...
SemSearch --> Filter["过滤已存在药材"]
Filter --> Merge["去重并合并(语义优先)"]
Merge --> BuildCtx
...
- **更新** 特点：不阻塞接口响应，保证一致性且提升用户体验。
- **更新** 增强的错误处理：在向量同步失败时记录日志但不影响主流程。
...
**更新** 经过增强的错误处理、缓存机制和降级策略，系统在各种异常情况下的稳定性和可靠性得到显著提升，为用户提供更加健壮的语义检索服务。
...
- **更新** 新增配置项
  - RATE_LIMIT_WINDOW_MS：API限流窗口时间
